 ...
 * (Admin, GDR Masters, GDR Mods, GDR Graphers x Masters, Graphers).
 * @returns {boolean}
 */
function isStaff() {
    return /\badmin\b/g.test(document.body.className) || /(g1|g2|g3|g4)\b/g.test(document.body.className);
}
...
/**
 * Attende che una condizione diventi vera, poi esegue il callback.
 * Utile per aspettare che variabili asincrone (es. chiavi API) siano disponibili.
 *
 * @param {Function} condition Funzione che restituisce true quando la condizione è soddisfatta
 * @param {Function} callback Funzione da eseguire quando la condizione è vera
 * @param {number} [interval=100] Intervallo di polling in ms (default: 100)
 * @param {number} [timeout=10000] Timeout massimo in ms, dopo il quale smette di attendere (default: 10s)
 *
 * @example
 * // Attende che la master key sia disponibile
 * HxHFramework.utilities.waitFor(
 *     function() { return HxHFramework.constants.JSONBIN_MASTER_KEY !== null; },
 *     function() { console.log('Chiave pronta!'); }
 * );
 */
function waitFor(condition, callback, interval, timeout) {
    ...
    function check() {
        if (condition()) {
            ...
            console.warn('[HxHFramework] waitFor: timeout raggiunto senza che la condizione fosse soddisfatta.');
        } else {
            elapsed += interval;
...
/**
 * Stato del forum nella classifica Top Forum.
 *
 * Al primo accesso del mese alla home (dove ci sono le statistiche),
 * la posizione viene letta da #top e salvata su Firebase in
 * topforum/AAAA-MM. Le regole Firebase impediscono di modificare il
 * record una volta creato, quindi il valore resta fisso per tutto il mese.
 * Da qualsiasi pagina, get() restituisce il valore del mese corrente.
 *
 * Risultato passato ai callback:
 * {
 *     mese:      '2026-10',
...
/**
 * Chiave del mese corrente secondo l'ora italiana, es. '2026-10'.
 * @returns {string}
 */
...
/**
 * Legge la posizione in classifica dalle statistiche della pagina.
 * @param {Document} [doc] Documento da leggere (default: pagina corrente)
 * @returns {number|null}
 */
function topForumLeggiPagina(doc) {
    doc = doc || document;
    var b = doc.querySelector('#top a b');
    var pos = b ? parseInt(b.textContent, 10) : NaN;
...
function topForumRisultato(mese, record) {
    ...
    var ris = {
        mese:      mese,
        rilevato:  ok,
        posizione: ok ? record.posizione : null,
...
function topForumSalvaCache(mese, record) {
    storageSet(TOPFORUM_CACHE_KEY, { mese: mese, posizione: record.posizione });
}
...
function topForumLeggiRecord(url, fine) {
    fetch(url)
        ...
        .catch(function(e) {
            console.error('[HxHFramework] topForum lettura:', e);
            fine(null);
        });
}
...
function topForumScrivi(url, mese, pos, fine) {
    ...
    fetch(url, { method: 'PUT', body: JSONtoString(record) })
        .then(function(r) {
            ...
            // Scrittura rifiutata: quasi certamente un altro utente ha creato
            // il record un attimo prima. Si rilegge quello.
        ...
        .catch(function(e) {
            console.error('[HxHFramework] topForum scrittura:', e);
            fine(topForumRisultato(mese, null));
        });
}
...
function topForumCarica(mese, fine) {
    ...
    topForumLeggiRecord(url, function(record) {
        ...
        // Record del mese assente: si può creare solo dalla home,
        // dopo che le statistiche sono state caricate.
        topForumQuandoPronto(function() {
            var pos = isHome() ? topForumLeggiPagina(document) : null;
            if (pos === null) {
                fine(topForumRisultato(mese, null));
                return;
            }
...
/**
 * Restituisce lo stato Top Forum del mese corrente.
 * Usa la cache locale se già presente per questo mese, altrimenti
 * interroga Firebase (e, se si è in home e il record manca, lo crea).
 *
 * @param {Function} callback Chiamata con l'oggetto risultato (vedi sopra)
 *
 * @example
 * HxHFramework.topForum.get(function(tf) {
 *     if (tf.dentro) { ... }
 * });
 */
function topForumGet(callback) {
    ...
    // Se una richiesta è già in corso, ci si accoda invece di farne un'altra
    if (topForumInAttesa) {
        topForumInAttesa.push(callback);
        return;
    }
    ...
    topForumCarica(mese, function(ris) {
        ...
        for (var i = 0; i < lista.length; i++) {
            try { lista[i](ris); }
            catch(e) { console.error('[HxHFramework] topForum callback:', e); }
        }
    });
}

/**
 * Svuota la cache locale (utile per test o dopo aver corretto
 * a mano un record su Firebase).
 */
function topForumSvuotaCache() {
    storageRemove(TOPFORUM_CACHE_KEY);
}
...
// del mese viene creato al primo accesso anche se nessun altro
// script ha chiamato get().
topForumQuandoPronto(function() {
    if (isHome()) topForumGet(function() {});
});
